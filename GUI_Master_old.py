...
root.title("Image Foregery Detection")
...
#
lbl = tk.Label(root, text="Image Foregery Detection", font=('times', 35,' bold '), width=65,height=1,bg="#152238",fg="white")
lbl.place(x=0, y=0)


frame_alpr = tk.LabelFrame(root, text=" --Image_Process-- ", width=920, height=120, bd=5, font=('times', 14, ' bold '),bg="brown")
frame_alpr.grid(row=0, column=0, sticky='nw')
frame_alpr.place(x=200, y=680)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        if cell == 0:
            Cd="ORIGINAL Image Detected."
            
        elif cell == 1:
            Cd="TAMPERED Image Detected."
            
        elif cell == 2:
            Cd="FORGED Image Detected"
        ...
        return A
    img = open(fn)    
    myfile = open("C:/Image Foregery Detection 100% Code/Image Foregery Detection 100% Code/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='C:/Image Foregery Detection 100% Code/Image Foregery Detection 100% Code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
